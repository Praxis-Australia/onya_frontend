...
class _PaymentsPageState extends State<PaymentsPage> {
  ...
  Future<List<MyTransactionCard>> _createTransactionList(transactions) async {
    ...
    for (var transaction in transactions) {
      await FirebaseFunction.read('transactions', transaction).then((value) => {
            if ((value!['post_date']) is String)
              {
                value!['post_date'] = Timestamp.fromDate(
            ...
            cards.add(MyTransactionCard(
              ...
              date: DateFormat('yyyy-MM-dd')
                  .format(fromTimestampToDateTime(value!['post_date'])),
              charityPref: 'ello',
            )),
          });
    }
  ...
  Future<List<MyTransactionCard>> _getTransactions(id) async {
    Future<Map<String, dynamic>?> transactionList =
        FirebaseFunction.read('users', id);
    var transactions = (await transactionList)!['transactions'];
    print(transactions);
    var detailedTransactionList =
        await _createTransactionList(await transactions);
